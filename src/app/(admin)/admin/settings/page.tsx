...
const DEFAULT_SETTINGS: StoreSettings = {
    ...
    shipping: { distanceTable: [], fixedAreas: [], freeShippingAbove: 0 }
};
...
export default function AdminSettingsPage() {
  ...
  const copyLocationLink = () => {
      const link = `http://googleusercontent.com/maps.google.com/?q=${settings.location.lat},${settings.location.lng}`;
      navigator.clipboard.writeText(link);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
  };

  const updatePayment = (k: any, f: any, v: any) => setSettings(p => ({...p, paymentMethods: {...p.paymentMethods, [k]: {...p.paymentMethods[k], [f]: v}}}));
  ...
  const updateDistRule = (i: number, f: keyof ShippingDistanceRule, v: number) => {const nw=[...settings.shipping.distanceTable];nw[i]={...nw[i],[f]:v};setSettings(p=>({...p,shipping:{...p.shipping,distanceTable:nw}}));};
  ...
  const updateFixedArea = (i: number, f: string, v: any) => {const nw=[...settings.shipping.fixedAreas];nw[i]={...nw[i],[f]:v};setSettings(p=>({...p,shipping:{...p.shipping,fixedAreas:nw}}));};
    ...
    <div className="max-w-5xl mx-auto pb-20 p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-white p-4 rounded-xl border shadow-sm">
          ...
          <div className="flex items-center gap-4">
              ...
              >
                  ...
                  <span className="font-bold text-sm uppercase">{settings.isOpen ? 'Loja Aberta' : 'Loja Fechada'}</span>
                  {settings.isOpen ? <Unlock size={16}/> : <Lock size={16}/>}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in">
            <div className="space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
                    ...
                    <div><label className="text-xs font-bold text-gray-500 uppercase">CNPJ</label><input className="w-full p-2 border rounded" value={settings.cnpj} onChange={e => setSettings({...settings, cnpj: e.target.value})} /></div>
                    <div><label className="text-xs font-bold text-gray-500 uppercase">Email</label><input className="w-full p-2 border rounded" value={settings.email} onChange={e => setSettings({...settings, email: e.target.value})} /></div>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
      ...
      {activeTab === 'payment' && (
          <div className="space-y-6 animate-in fade-in">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                  ...
                  <div className="space-y-3">{Object.entries(settings.paymentMethods).map(([key, method]) => (<div key={key} className="flex gap-4 items-center p-3 border rounded bg-gray-50"><input type="checkbox" checked={method.active} onChange={e => updatePayment(key, 'active', e.target.checked)} className="w-5 h-5"/><div className="flex-1"><label className="text-xs font-bold text-gray-400 uppercase">Nome</label><input className="w-full p-2 border rounded bg-white" value={method.label} onChange={e => updatePayment(key, 'label', e.target.value)}/></div><div className="w-24"><label className="text-xs font-bold text-red-400 uppercase">Taxa %</label><input type="number" className="w-full p-2 border rounded bg-white" value={method.feePercent} onChange={e => updatePayment(key, 'feePercent', Number(e.target.value))}/></div></div>))}</div>
              </div>
          </div>
      )}
      ...
      {activeTab === 'shipping' && (
          <div className="space-y-8 animate-in fade-in">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                  <div className="flex justify-between items-center mb-4"><div><h2 className="font-bold text-lg text-slate-800">2. Outras Localidades</h2></div><button onClick={addFixedArea} className="text-blue-600 text-xs font-bold flex gap-1 hover:underline"><Plus size={14}/> Add Setor</button></div>
                  <div className="space-y-3">{settings.shipping.fixedAreas.map((a, i) => (<div key={a.id} className="flex flex-col md:flex-row gap-3 p-3 border rounded-xl bg-gray-50 items-center"><input className="flex-1 p-2 border rounded font-bold text-sm" placeholder="Nome do Setor" value={a.name} onChange={e => updateFixedArea(i, 'name', e.target.value)}/><select className="p-2 border rounded text-sm bg-white" value={a.type} onChange={e => updateFixedArea(i, 'type', e.target.value)}><option value="fixed">Valor Fixo</option><option value="km_plus_tax">Km + Taxa</option></select>{a.type === 'fixed' ? <div className="flex items-center gap-1"><span className="text-xs font-bold text-gray-500">R$</span><input type="number" className="w-20 p-2 border rounded font-bold text-green-600" value={a.price} onChange={e => updateFixedArea(i, 'price', Number(e.target.value))}/></div> : <div className="flex items-center gap-2"><span className="text-[10px] uppercase font-bold text-blue-600">Taxa:</span><input type="number" className="w-16 p-2 border rounded font-bold" value={a.tax || 0} onChange={e => updateFixedArea(i, 'tax', Number(e.target.value))}/></div>}<button onClick={() => removeFixedArea(i)} className="text-red-400 p-2"><Trash2 size={16}/></button></div>))}</div>
              </div>
          </div>
      )}
    </div>
