...
import { User, ShoppingCart, Phone, MapPin, Loader2, FileText, History } from "lucide-react";
            ...
            {customers.map((customer) => (
                <div key={customer.id} className={`bg-white p-4 rounded-xl shadow-sm border-l-4 flex flex-col md:flex-row justify-between md:items-center gap-4 ${customer.clientType === 'monthly' ? 'border-l-purple-600' : 'border-l-gray-300'}`}>
                    <div className="flex items-center gap-4 min-w-[250px]">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl text-white ${customer.clientType === 'monthly' ? 'bg-purple-600' : 'bg-gray-400'}`}>{customer.name ? customer.name.charAt(0).toUpperCase() : <User/>}</div>
                        <div>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 w-full justify-end">
                        ...
                        <div className="flex gap-6 w-full md:w-auto justify-around md:justify-start"><div className="text-center"><p className="text-[10px] font-bold text-gray-400 uppercase">Pedidos</p><p className="font-bold text-gray-800 flex items-center gap-1 justify-center"><ShoppingCart size={14} className="text-pink-500"/> {customer.orderCount}</p></div><div className="text-center"><p className="text-[10px] font-bold text-gray-400 uppercase">Gasto Total</p><p className="font-bold text-green-600">{customer.totalSpent.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</p></div></div>

                        <div className="flex gap-2">
                            {/* HISTÓRICO */}
                            <Link href={`/admin/customers/${customer.id}/history`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg border border-blue-200" title="Histórico Completo">
                                <History size={20}/>
                            </Link>

                            {/* BOLETA (Só se for mensalista) */}
                            {customer.clientType === 'monthly' && (
                                <Link href={`/admin/customers/${customer.id}/invoice`} className="p-2 text-green-600 hover:bg-green-50 rounded-lg border border-green-200" title="Gerar Boleta">
                                    <FileText size={20}/>
                                </Link>
                            )}
                        </div>
