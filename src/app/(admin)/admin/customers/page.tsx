...
export default function CustomersPage() {
  ...
  // CORREÇÃO: Preço manual agora considera Revenda também
  ...
  const addCustomItem = () => { const name = prompt("Descrição:"); const priceStr = prompt("Valor R$:"); if (name && priceStr) { const price = parseFloat(priceStr.replace(',', '.')); if (!isNaN(price)) setManualItems([...manualItems, { id: crypto.randomUUID(), name, price, quantity: 1 }]); } };
  ...
  const handleSubmitEntry = async () => {
      ...
      try {
          await addDoc(collection(db, "orders"), {
              ...
              description: manualObservation || (entryType === 'credit' ? 'Crédito' : 'Manual')
          });
          alert("Lançado!"); setIsModalOpen(false);
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 w-full justify-end">
                        <div className="w-full md:w-auto">
                            ...
                            <select className={`w-full md:w-40 p-2 rounded border text-sm font-bold outline-none ${customer.clientType === 'monthly' ? 'text-purple-700 bg-purple-50 border-purple-200' : customer.clientType === 'reseller' ? 'text-orange-700 bg-orange-50 border-orange-200' : 'text-gray-700'}`} value={customer.clientType} onChange={(e) => changeClientType(customer.id, e.target.value)}>
                                ...
                                <option value="reseller">💼 Revendedor</option> {/* OPÇÃO NOVA */}
                            </select>
                        </div>
                        ...
                        <div className="flex gap-2">
                            {(customer.clientType === 'monthly' || customer.clientType === 'reseller') && <button onClick={() => openEntryModal(customer)} className="p-2 bg-purple-100 text-purple-700 hover:bg-purple-200 rounded-lg border border-purple-200" title="Novo Lançamento"><PlusCircle size={20}/></button>}
        ...
        {isModalOpen && selectedCustomer && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in">
                <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                    <div className="p-6 border-b flex justify-between items-center"><div><h2 className="text-xl font-bold text-gray-800">Lançamento Manual</h2><p className="text-sm text-gray-500">Cliente: {selectedCustomer.name}</p></div><button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full"><PlusCircle className="rotate-45" size={24}/></button></div>
                    <div className="p-6 overflow-y-auto flex-1 space-y-6">
                        <div className="flex bg-gray-100 p-1 rounded-xl"><button onClick={() => setEntryType('debit')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${entryType === 'debit' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500'}`}>Débito (+)</button><button onClick={() => setEntryType('credit')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${entryType === 'credit' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500'}`}>Crédito (-)</button></div>
                        ...
                        <div><label className="font-bold text-sm mb-1 block">Observação</label><textarea className="w-full p-2 border rounded-lg text-sm bg-gray-50" rows={2} placeholder="Detalhes..." value={manualObservation} onChange={e => setManualObservation(e.target.value)}/></div>
                    </div>
