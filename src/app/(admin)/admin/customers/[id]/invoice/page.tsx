...
import { Loader2, Printer, ArrowLeft, Phone, Mail, MapPin, FileText } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-8 print:p-0 print:bg-white font-sans">
      ...
      <div className="max-w-4xl mx-auto mb-6 flex justify-between items-center print:hidden">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 font-bold bg-white px-4 py-2 rounded-lg shadow-sm border transition text-sm md:text-base">
            <ArrowLeft size={18}/> <span className="hidden md:inline">Voltar</span>
        </button>
        <button onClick={() => window.print()} className="flex items-center gap-2 bg-slate-900 text-white px-4 md:px-6 py-2 rounded-lg font-bold shadow-md hover:bg-slate-800 transition text-sm md:text-base">
            <Printer size={18}/> <span className="hidden md:inline">Imprimir / Salvar PDF</span><span className="md:hidden">PDF</span>
        </button>
      </div>

      {/* --- FOLHA DA FATURA --- */}
      <div className="max-w-4xl mx-auto bg-white p-6 md:p-12 rounded-xl shadow-xl print:shadow-none print:w-full print:max-w-none print:p-0">
        
        {/* Cabeçalho Responsivo */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-gray-100 pb-6 mb-6 gap-6 md:gap-0">
            <div className="flex gap-4 items-center">
                <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border border-gray-100 overflow-hidden relative shadow-sm flex-shrink-0">
                    <img src={COMPANY_INFO.logoUrl} alt="Logo" className="w-full h-full object-cover"/>
                </div>
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-900 leading-none mb-1">{COMPANY_INFO.name}</h1>
                    <p className="text-xs md:text-sm text-pink-600 font-bold uppercase tracking-widest mb-2">{COMPANY_INFO.subtitle}</p>
            ...
            <div className="text-left md:text-right w-full md:w-auto bg-gray-50 md:bg-transparent p-3 md:p-0 rounded-lg">
                <h2 className="text-3xl md:text-5xl font-bold text-slate-800 tracking-tight mb-1">FATURA</h2>
                <div className="inline-block md:block">
                    <p className="text-xs text-gray-500 font-bold uppercase inline md:block mr-2 md:mr-0">Emissão:</p>
                    <p className="text-sm font-bold text-slate-700 inline md:block">{new Date().toLocaleDateString('pt-BR')}</p>
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-6 mb-8">
            <div className="flex-1 bg-slate-50 p-4 md:p-5 rounded-lg border border-slate-100">
                ...
                <h3 className="text-lg md:text-xl font-bold text-slate-800 mb-1">{client.name}</h3>
            ...
            <div className="w-full md:w-64 bg-slate-900 text-white p-4 md:p-5 rounded-lg shadow-lg flex flex-col justify-center text-center md:text-right">
                <p className="text-xs font-bold text-slate-400 uppercase mb-1">Total em Aberto</p>
                <p className="text-2xl md:text-3xl font-bold text-white">R$ {totalDebt.toFixed(2)}</p>
            </div>
        </div>
        ...
        <div className="flex justify-end pt-4">
            <div className="w-full md:w-1/2 space-y-3">
                <div className="flex justify-between text-sm text-gray-500 py-1 border-b border-dashed border-gray-200 pb-1">
                    <span>Subtotal Compras</span>
                    <span>R$ {orders.filter(o => o.total > 0).reduce((a, b) => a + Number(b.total), 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm text-green-600 py-1 border-b border-dashed border-gray-200 pb-1">
