...
export default function AdminOrdersPage() {
                      ...
                      if (groupSnap.exists()) {
                          ...
                          (opts as Option[]).forEach(selectedOpt => {
                              ...
                              if (idx !== -1 && optionsList[idx].stock !== null) {
                                  // Multiplica pelo multiplier (1 ou -1) e pela quantidade do item pai
  ...
  const getStatusStyle = (status: string) => {
      switch(status) {
          ...
          case 'finalizado': return 'border-l-green-600 bg-white opacity-70 grayscale';
      ...
      <div className="flex flex-col gap-6 mb-8">
          <div className="flex justify-between items-center">
            ...
            <div className="text-xs text-gray-400 flex items-center gap-1">
                <Bell size={14} className="text-pink-500 animate-pulse"/> Som ON
            </div>
          </div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between gap-6">
                        ...
                        <div className="flex flex-col justify-center gap-3 min-w-[100%] lg:min-w-[200px] border-t lg:border-t-0 lg:border-l pt-4 lg:pt-0 lg:pl-6 border-gray-200">
                            <div className="text-center">
                                <span className={`text-sm font-black uppercase tracking-widest ${order.status === 'em_aberto' ? 'text-yellow-600' : order.status === 'produzindo' ? 'text-blue-600' : order.status === 'entrega' ? 'text-orange-600' : 'text-green-600'}`}>{getStatusLabel(order.status)}</span>
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2">
                                <button onClick={() => updateStatus(order, 'produzindo')} className="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 text-xs font-bold flex flex-col items-center"><ChefHat size={18} className="mb-1"/> Produzir</button>
                                <button onClick={() => updateStatus(order, 'entrega')} className="p-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 text-xs font-bold flex flex-col items-center"><Truck size={18} className="mb-1"/> Rota</button>
                                <button onClick={() => updateStatus(order, 'finalizado')} className="col-span-2 p-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 text-xs font-bold flex items-center justify-center gap-2"><CheckCircle size={18}/> FINALIZAR</button>
