...
import { 
  ...
  ShoppingBag, Gift, Calendar, Star, List, Tag
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  // --- FUNÇÕES DE COMPLEMENTOS (CORRIGIDAS) ---
  ...
  const handleSaveGroup = async (e: React.FormEvent) => { 
      e.preventDefault(); 
      if (!editingGroup.title) return alert("O título do grupo é obrigatório."); 
      try { 
          const payload = { 
  ...
  const delGroup = async (id: string) => { if (confirm("Excluir este grupo?")) { setGroups(prev => prev.filter(g => g.id !== id)); await deleteGroup(id); } };
  ...
  const updateOption = (idx: number, field: keyof Option, val: any) => { 
      setEditingGroup(prev => {
          const opts = [...(prev.options || [])];
          // Antes estava opts.splice(idx, 1) que removia o item!
  ...
  return (
    <div className="space-y-6 pb-20 relative">
      ...
      {loading ? <div className="p-20 text-center flex justify-center"><Loader2 className="animate-spin text-pink-600"/></div> : (
        <>
            {activeTab === 'products' && (
                <div>
                                                ...
                                                <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="p-4 flex gap-3 items-center">
                                                        <div className="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0 border border-gray-200">
                                                            {p.imageUrl ? <img src={p.imageUrl} className="w-full h-full object-cover"/> : <ImageOff size={14} className="m-auto text-gray-400"/>}
                                                        </div>
                                                    ...
                                                    <td className="p-4 text-right w-36">
                                                        <div className="flex justify-end gap-1">
                                                            <button 
                                                                ...
                                                                className="p-1.5 text-pink-500 hover:bg-pink-50 rounded border border-transparent hover:border-pink-200 transition" 
                                                                title="Criar Oferta"
                                                            >
                                                                <Tag size={16}/>
                                                            </button>

                                                            <Link href={`/admin/products/${p.id}`} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded border border-transparent hover:border-blue-200 transition"><Pencil size={16}/></Link>
                                                            <button onClick={() => delProd(p.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded border border-transparent hover:border-red-200 transition"><Trash2 size={16}/></button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
            ...
            <div className="bg-white w-full max-w-2xl max-h-[90vh] rounded-xl shadow-2xl flex flex-col">
                                    ...
                                    <div key={idx} className={`flex gap-2 items-center p-2 rounded border ${isLinked ? 'bg-blue-50 border-blue-200' : 'bg-white'}`}>
                                        <div className="flex-1">
                                            <input 
                                                disabled={isLinked} 
                                                placeholder="Nome" 
                                                className="w-full p-1 bg-transparent border-b border-dashed text-sm font-medium focus:outline-none" 
                                                value={opt.name} 
                ...
                <div className="p-4 border-t bg-gray-50 flex flex-col md:flex-row gap-3 justify-between items-center">
                    <div className="flex gap-2 w-full md:w-auto">
                        <button onClick={() => addOptionToGroup('simple')} className="flex-1 md:flex-none px-3 py-2 border bg-white rounded text-sm font-medium hover:bg-gray-100">+ Simples</button>
                        <select 
                            ...
                            onChange={(e) => { 
                                if(e.target.value) { 
                                    addOptionToGroup('product', e.target.value); 
                                    e.target.value = ""; 
                                } 
                            }}
                        >
