...
import { Trash2, ArrowLeft, Send, MapPin, Search, Loader2, ShoppingBag, CreditCard, FileText, CheckCircle, Plus, Minus, AlertTriangle, Phone, QrCode, Copy, X, Link as LinkIcon, Banknote, Save, Pencil, Clock, Ticket, Percent } from "lucide-react";
...
export default function CartPage() {
  ...
  // Estados de Valores
  const [baseShippingPrice, setBaseShippingPrice] = useState(0); // Frete REAL calculado (Rota/Tabela)
  const [shippingPrice, setShippingPrice] = useState(0); // Frete FINAL (Pode ser 0 ou reduzido por cupom)
  const [discountValue, setDiscountValue] = useState(0); // Valor do desconto em R$ (Itens)
  ...
  const [activeCoupon, setActiveCoupon] = useState<any>(null); // Objeto completo do cupom
  ...
  useEffect(() => {
    ...
    const validate = async () => {
        ...
        for (const item of items) {
            if (!item.id || item.category === 'combo' || item.category === 'promo') continue;
            try {
  ...
  // 4. CÁLCULO DE FRETE BASE (ROTA DE CARRO)
  useEffect(() => {
    // Se for Retirada OU Conta Mensal, frete base é zero
    if (deliveryMethod === 'pickup' || paymentMethod === 'conta_aberta') { 
        setBaseShippingPrice(0);
    ...
    const applyDistancePricing = (distKm: number) => {
        ...
        // Frete Grátis por Valor Mínimo (Configuração da Loja)
        if(storeSettings.shipping.freeShippingAbove && storeSettings.shipping.freeShippingAbove > 0 && cartTotal >= storeSettings.shipping.freeShippingAbove) {
            finalPrice = 0;
            setShippingDetails("Frete Grátis (Valor Mínimo)");
        } else {
            setShippingDetails(`${distKm.toFixed(1)} km (Rota)`);
        }
        setBaseShippingPrice(finalPrice);
    };

    // --- LÓGICA 1: PLANO DIRETOR ---
    ...
    // --- LÓGICA 2: OUTRAS LOCALIDADES ---
        ...
        if (area) {
            ...
            if (area.type === 'fixed') {
                setBaseShippingPrice(area.price);
  ...
  // 5. APLICAÇÃO DE CUPOM E CÁLCULO FINAL
  useEffect(() => {
      let finalShip = baseShippingPrice;
      let itemDisc = 0;

      if (activeCoupon) {
          // 1. CUPOM DE FRETE
          if (activeCoupon.couponType === 'shipping' || !activeCoupon.couponType) { // Fallback para cupons antigos
              const limit = activeCoupon.shippingLimit;
              if (limit) {
                  // Se tem limite (ex: frete grátis até 10,00). 
                  // Se frete for 15, desconta 10, paga 5.
                  const discount = Math.min(baseShippingPrice, limit);
                  finalShip = Math.max(0, baseShippingPrice - discount);
              } else {
                  // Sem limite = 100% grátis
                  finalShip = 0;
              }
          } 
          // 2. CUPOM DE PORCENTAGEM
          else if (activeCoupon.couponType === 'percent') {
              const percent = activeCoupon.discountPercent || 0;
              itemDisc = cartTotal * (percent / 100);
          }
      }

      setShippingPrice(finalShip);
      setDiscountValue(itemDisc);

  }, [activeCoupon, baseShippingPrice, cartTotal]);
  ...
  const applyCoupon = async () => {
      ...
      try {
          // Busca promoção que contenha esse código no array 'codes'
          ...
          const promoDoc = snap.docs[0].data();
          setActiveCoupon({ 
              code: couponCode, 
              id: snap.docs[0].id,
              couponType: promoDoc.couponType,
              shippingLimit: promoDoc.shippingLimit,
              discountPercent: promoDoc.discountPercent
          });
          setCouponCode("");
      } catch (e) { console.error(e); setCouponError("Erro ao validar."); }
  };

  const removeCoupon = () => { setActiveCoupon(null); setCouponCode(""); };
  ...
  const handleOpenPix = () => {
      const total = Math.max(0, cartTotal + shippingPrice - discountValue);
  ...
  const handleCheckout = async () => {
    ...
    // TOTAL FINAL CONSIDERANDO DESCONTOS
    const finalTotal = Math.max(0, cartTotal + shippingPrice - discountValue);
    ...
    try {
        ...
        await runTransaction(db, async (transaction) => {
            // Verifica e Queima Cupom (Atomicamente)
            if (activeCoupon) {
                const promoRef = doc(db, "promotions", activeCoupon.id);
                const promoSnap = await transaction.get(promoRef);
                if (promoSnap.exists()) {
                    const codes = promoSnap.data().codes || [];
                    if (!codes.includes(activeCoupon.code)) throw new Error("Cupom já utilizado por outro usuário ou expirado.");
                    // Remove o cupom usado do array
                    transaction.update(promoRef, { codes: arrayRemove(activeCoupon.code) });
                } else {
                    throw new Error("Promoção de cupom não encontrada.");
                }
            }
            ...
            for (const item of items) {
                // SE FOR COMBO, BAIXA OS ITENS INTERNOS
                if (item.category === 'combo' && (item as any).comboIds) {
                    const comboIds = (item as any).comboIds as string[];
                    comboIds.forEach(id => addProdDec(id, item.quantity));
                } 
                // SE FOR NORMAL OU PROMOÇÃO (ID ÚNICO), BAIXA DIRETO
                else if (item.id) {
            ...
            transaction.set(newOrderRef, {
                ...
                deliveryMethod: deliveryMethod, createdAt: serverTimestamp(), 
                shippingPrice: shippingPrice, 
                discountValue: discountValue, // Salva o valor do desconto
                address: finalAddress, 
                isPaid: false,
                couponCode: activeCoupon?.code || null // Salva qual cupom foi usado
            });
        });
        ...
        if(discountValue > 0) msg += `📉 Desconto: - R$ ${discountValue.toFixed(2)}\n`;
  ...
  return (
    <div className="pb-40 pt-2 px-4 max-w-2xl mx-auto">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 mb-4 shadow-sm">
          <h2 className="font-bold text-sm flex gap-2 items-center mb-3 text-slate-700"><Ticket size={16} className="text-pink-600"/> Cupom de Desconto</h2>
          {activeCoupon ? (
              <div className="flex justify-between items-center bg-green-50 p-3 rounded-lg border border-green-200 animate-in slide-in-from-left">
                  <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-green-600 ml-6">
                          {activeCoupon.couponType === 'percent' ? `Desconto de ${activeCoupon.discountPercent}% nos itens` : 
                           activeCoupon.shippingLimit ? `Frete Grátis até R$ ${activeCoupon.shippingLimit}` : 'Frete Grátis Total'}
                      </span>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg safe-area-bottom z-40">
        <div className="max-w-2xl mx-auto space-y-2">
             ...
             <div className="text-xs text-gray-500 space-y-1 pb-2 border-b border-gray-100">
                ...
                {discountValue > 0 && (
                    <div className="flex justify-between text-green-600 font-bold"><span>Desconto:</span> <span>- R$ {discountValue.toFixed(2)}</span></div>
                )}
                
                <div className="flex justify-between">
                    <span>Frete:</span> 
                    {deliveryMethod === 'pickup' ? <span>Grátis</span> : 
                     shippingPrice < baseShippingPrice ? 
                     <span className="flex gap-2"><s className="text-red-400">R$ {baseShippingPrice.toFixed(2)}</s> <b className="text-green-600">R$ {shippingPrice.toFixed(2)}</b></span> : 
                     <span>{shippingDetails || `R$ ${baseShippingPrice.toFixed(2)}`}</span>
                    }
                </div>
             </div>
             ...
             <div className="flex justify-between font-bold text-lg text-gray-800 pt-1">
                 ...
                 <span className="text-green-600">R$ {Math.max(0, cartTotal + shippingPrice - discountValue).toFixed(2)}</span>
             </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in">
              <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 relative animate-in zoom-in-95 duration-300">
                  ...
                  <div className="text-center mb-6">
                      ...
                      <p className="text-sm text-stone-500">Valor Total: <strong className="text-emerald-600">R$ {Math.max(0, cartTotal + shippingPrice - discountValue).toFixed(2)}</strong></p>
                  </div>
