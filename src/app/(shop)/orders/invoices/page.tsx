...
export default function MyInvoicesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // Busca todos os pedidos do usuário ordenados
        const q = query(
        ...
        // FILTRO CRÍTICO: Apenas o que é "Conta Aberta" e NÃO foi pago (finalizado)
  ...
  return (
    <div className="pb-20 pt-6 px-4 max-w-2xl mx-auto">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="font-bold text-xl">Minha Fatura em Aberto</h1>
      </div>

      <div className="bg-gradient-to-r from-purple-700 to-indigo-600 rounded-2xl p-6 text-white shadow-lg mb-6">
        <p className="opacity-80 text-sm font-medium mb-1">Total a Pagar</p>
        <h2 className="text-4xl font-bold">R$ {openTotal.toFixed(2)}</h2>
        <p className="text-xs mt-2 opacity-70">Soma de todos os pedidos em aberto.</p>
      </div>
      ...
      <div className="space-y-3">
        {orders.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-xl border border-dashed">
                <p className="text-gray-400 font-medium">Tudo pago! Nenhuma pendência.</p>
            </div>
        ) : orders.map(order => {
                ...
                <div key={order.id} className="bg-white p-4 rounded-xl border shadow-sm flex justify-between items-start">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] font-bold text-purple-600 bg-purple-100 px-1.5 rounded">#{order.shortId || '---'}</span>
                        </div>
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-[10px] font-bold uppercase text-yellow-600">Pendente</p>
                    </div>
                </div>
            )
         })
