    ...
    <div className="space-y-8 pb-24">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-stone-900 shadow-xl shadow-stone-200 h-32 flex items-center justify-center text-center p-6">
          ...
          <div className="relative z-10 text-white">
              <h1 className="text-2xl font-bold mb-2 text-shadow">Eventos & Festas</h1>
              <p className="text-stone-100 text-xs font-medium">Kits para celebrar.</p>
          </div>
      </div>
      ...
      {products.length === 0 && <div className="text-center py-20 text-stone-400"><Star size={48} className="mx-auto mb-2 opacity-20"/><p>Nenhum evento especial ativo.</p></div>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/80 p-4 animate-in fade-in backdrop-blur-sm">
            ...
            <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl relative flex flex-col md:flex-row animate-in slide-in-from-bottom-8">
                ...
                <div className="w-full md:w-3/5 bg-black flex flex-col justify-center relative">
                    ...
                    <div className="p-4 bg-black/40 backdrop-blur-md overflow-x-auto flex gap-2 justify-center">
                        {[selectedProduct.imageUrl, ...(selectedProduct.gallery || [])].filter(Boolean).map((url, idx) => (
                            <button key={idx} onClick={() => setCurrentImage(url)} className={`w-14 h-14 rounded-lg border-2 overflow-hidden flex-shrink-0 transition-all ${currentImage === url ? 'border-orange-500 opacity-100 scale-110' : 'border-transparent opacity-60 hover:opacity-100'}`}><img src={url} className="w-full h-full object-cover"/></button>
                        ))}
                    </div>
                </div>
                <div className="w-full md:w-2/5 p-8 flex flex-col bg-white overflow-y-auto">
                    ...
                    {selectedProduct.videoUrl && (<a href={selectedProduct.videoUrl} target="_blank" rel="noopener noreferrer" className="mb-6 flex items-center gap-3 p-4 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 transition border border-red-100 group"><div className="bg-white p-2 rounded-full shadow-sm group-hover:scale-110 transition"><PlayCircle size={24}/></div><div className="text-sm font-bold">Vídeo do Evento</div></a>)}
                    <div className="mt-auto border-t border-stone-100 pt-6"><button onClick={() => openWhatsApp(selectedProduct)} className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-800 transition shadow-lg shadow-stone-200 hover:-translate-y-1"><MessageCircle size={22}/> Falar com Buffet</button></div>
                </div>
            </div>
        </div>
      )}
