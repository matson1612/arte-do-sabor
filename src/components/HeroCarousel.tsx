...
export default function HeroCarousel({ products }: { products: Product[] }) {
  ...
  return (
    <div className="relative w-full h-[280px] md:h-[400px] rounded-3xl overflow-hidden shadow-xl shadow-stone-200 mb-8 group bg-stone-100">
          ...
          <div key={product.id} className="min-w-full h-full relative">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-stone-900/90 via-stone-900/20 to-transparent flex items-end p-8">
              <div className="text-white transform transition-all duration-500 translate-y-0 max-w-2xl">
                <span className="bg-pink-500 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-2 inline-block shadow-sm">Destaque</span>
      ...
      <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 backdrop-blur-md p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition hover:bg-white hover:text-stone-800"><ChevronLeft size={24}/></button>
      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 backdrop-blur-md p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition hover:bg-white hover:text-stone-800"><ChevronRight size={24}/></button>

      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
        {highlights.map((_, idx) => (
          <div key={idx} onClick={() => setCurrentIndex(idx)} className={`w-2 h-2 rounded-full cursor-pointer transition-all duration-300 ${currentIndex === idx ? "w-8 bg-pink-500" : "bg-white/50 hover:bg-white"}`}/>
        ))}
      </div>
    </div>
  );
